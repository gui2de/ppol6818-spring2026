//Abhinav Dutt Exp Design STATA 4 March 29
...
//Write your cd here
global yourcd "C:\Users\abhin\Downloads"
cd $yourcd

//Cleaning data before merging. Making sure that region, district, and ward are the same variable name in both datasets
...
//Using reclink2 to match 
use Tz_elec_15_progress, clear //2015 is designated as master file
...
------------+-----------------------------------
      Total |      2,912      100.00
*/
display (2115/1) + (626/2) + (126/3) + (40/4) + (5/5) //626 is divided by 2 as we only count one of each duplicate! Therefore, 126 divided by 3, 40 divided by 4, and 5 divided by 5. The sum gives the true number of wards in both 2010 and 2015!
//Answer 1.1: 2,481 wards exist both in 2010 and 2015
tab _merge //this table gives us the answer below
//Answer 1.2: 1,120 wards were only in 2015 (parentless); from _merge == 1. These wards were only found in master. 

display (3944 - 2481) - 1120
//343 wards in 2015 are still unaccounted for. Not an answer to a question but good to know!

//3333 (no. of wards in 2010) - 2481 (no. of wards from 2010 that were matched and accounted for) = 852 
//Answer 1.3: 852 wards in 2010 are still unaccounted for and unmatched (childless)
...
count if count_2012 == 2 & match_count == 1 //count_2012 will be 2 if ward from 2010 was split into two wards in 2015
//Answer 1.4: 453 wards were divided into two wards between 2010 and 2015

count if count_2012 > 2 & match_count == 1 //Now we need wards split into more than two parts
//Answer 1.5 32 wards

//1.6 Generate variables that calculate (Number of wards within a region that underwent division)/(total wards within in a region not counting duplicates)
...
//2.1 
capture program drop part2 //similar code to the one used by us in previous weeks
program define part2, rclass
	...
	replace t_confounder = 1 if rand_confounder < `r(p50)' //median used 
...
foreach n of numlist 10000 1000 100 10 { //we chose 4 different sample sizes
	//Loop is in descending order so the final datafile's observations are in ascending order according to sample size
...
dtable base_beta confounder_nc_beta confounder_c_beta mediator_nc mediator_c_beta collider_c_beta, by(N, nototals) nosample //output table that is given below
...
//README submitted with table and figures and explanations!
...
//Part 3
//ssc install shp2dta - do this if needed!
...
//Clean data based on website information
rename DP03_0001E employment
label variable employment "EMPLOYMENT STATUS: Population 16 years and over" //this is the variable we care about for our analysis and what we need to merge 
drop DP* //don't need all of these variables 
rename OBJECTID _ID //to allow for merge 
...
drop GEOID //variable not needed 
drop TRACTCE //variable not needed 

merge 1:1 _ID using dc_analysis.dta //now we have employment info in this dataset that we didn't have before as it came from a wholly separate dataset! Both datasets had 206 tracts and were for the year 2022 in DC - we made sure of this! 


spmap employment using dc_coord.dta, id(_ID) legend(size(*1)) legend(position(5)) fcolor(BuRd) clnumber(5) title("No. of people employed by Tract in DC (2022)") //Output needed for part 3 and PNG file included in submission!
