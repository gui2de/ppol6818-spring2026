...
* keep only candidate data lines
...
drop if departement=="cote d'ivoire"

duplicates drop departement, force

save "`dens'", replace

* Load household data, decode departement labels, merge density
use "q2_CIV_Section_0.dta", clear
decode b06_departemen, gen(departement)
replace departement = lower(itrim(trim(departement)))

merge m:1 departement using "`dens'", keepusing(density)
...
drop if missing(ward) | missing(party_clean)
