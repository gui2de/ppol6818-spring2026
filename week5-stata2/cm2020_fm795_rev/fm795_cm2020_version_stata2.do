...
***cm2020 note: adding my own directory in order to replicate the dofile
...
***Can use global command to make this more easily replicable 

save "q1_tanzania_temp_fm795.dta", replace

*** also recommend creating an alternative file so you are not editing the original files (which can create complications in editing)
...
***Overall, this looks right, you have the right number of observations and schools (138)
...
***cm2020 again linking to my directory
...
save "q2_CIV_populationdensity_fm795.dta", replace

***back to fm795 code below 
...
*******cm2020 code, creating a temporary file for the purposes of review

save "q2_CIV_Section_0_fm795.dta", replace
...
/*cm2020 notes: I tried running the code as it was and did not see a population variable in the final file, so something must have gone wrong with the merge. I then created a new temporary file from "q2_CIV_Section_0.dta" to see if that was the issue, and I got this response */

 decode department, gen(dept_str) // generating a string for department because it is a string variable i
> n the population density 
variable department not found


**In any event, the population density variable does not show up in the final merge. I (cm2020) will move on to the next question
...
***cm2020 linking to cm2020 directory and creating a working file 
...
save "q3_GPS_Data_fm795.dta", replace
...
***cm2020 The final file looks clean. However, I think there's a lot of variation in terms of distance from the centroid, so it may not be the most efficient assignment. 

***cm2020 code below

bysort enumerator_id: egen centroid_lat = mean(latitude)
bysort enumerator_id: egen centroid_lon = mean(longitude)


gen dlat = (latitude  - centroid_lat) * 111000
gen dlon = (longitude - centroid_lon) * 111000 * cos(latitude * _pi / 180)
gen dist_to_centroid_m = sqrt(dlat^2 + dlon^2)


bysort enumerator_id: egen max_dist_to_centroid = max(dist_to_centroid_m)
bysort enumerator_id: egen avg_dist_to_centroid = mean(dist_to_centroid_m)
...
import excel "q4_Tz_election_2010_raw.xls", sheet("Sheet1") cellrange(A5:K7927) firstrow

save "q4_Tz_election_2010_raw_fm795.dta", replace
...
***cm2020 note: Running this, I get the right number of observations! 
...
save "q5_school_location_fm795.dta", replace
...
***cm2020 Fm795 code below, but I want to use my working file instead to run the code
preserve
use "C:\Users\fabih\OneDrive\Desktop\MIDP\Spring 2026\Experimental Design\Stata 2\01_data\q5_school_location.dta"
...
***cm2020 Fm795 code but with my file

preserve
use "q5_school_location_fm795.dta"
keep Region Council Ward School
rename Region region
rename Ward ward
gen school_clean = lower(School)
...
***back to fm795 code, but again first changing file in order to be able to run code
...
***cm2020 note: nice, I couldn't figure out what to drop to get the right answer
